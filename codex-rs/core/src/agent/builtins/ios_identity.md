...
Every feature uses VIP+W Clean Architecture with strict layer separation:

  View (UIViewController)
    ↓ forwards user actions via <Feature>BusinessLogic protocol
  Interactor
    ↓ orchestrates logic, calls Workers
    ↓ sends results via <Feature>PresentationLogic protocol
  Presenter
    ↓ formats into ViewModels, sends via <Feature>DisplayLogic protocol
  View (displays ViewModel)

  Worker (called by Interactor)
    - Network/API calls (URLSession, Alamofire)
    - Data persistence (CoreData, SwiftData, UserDefaults)
    - Third-party SDKs (Analytics, Payments, etc.)

Layer communication is protocol-only — no concrete class dependencies:
  - View → Interactor: `<Feature>BusinessLogic` protocol
  - Interactor → Presenter: `<Feature>PresentationLogic` protocol
  - Presenter → View: `<Feature>DisplayLogic` protocol
  - Worker: `<Feature>WorkerProtocol`

Standard naming: LoginViewController, LoginInteractor, LoginPresenter, LoginWorker
Feature folder: Features/Login/ (all VIP+W files together)
...
- VIP+W back-references (Presenter → View, Router → View) are `weak var` (internal):
  These are wired by the scene factory AFTER all objects are created — `private` would
  prevent the factory from setting them:
...
### Scene Factory Wiring (UIApplicationDelegate)
- `UIApplicationDelegate.window` MUST be `var window: UIWindow?` — NOT `private var`:
  UIApplicationDelegate protocol requires an accessible window property.
- Presenter and Router use a no-arg `init()` — the scene factory sets `viewController`
  directly after creating the ViewController (avoids placeholder objects):
  let presenter = LoginPresenter()
  let router = LoginRouter()
  let viewController = LoginViewController(interactor: interactor, router: router)
  presenter.viewController = viewController   // internal weak var
  router.viewController = viewController      // internal weak var
- Interactor receives Presenter via init (Interactor is created before the VC):
  let presenter = LoginPresenter()
...
- Scaffold full VIP+W file set when asked to create a new feature
