...
function printBanner(project: XcodeProject | null): void {
  ...
  console.log(
    color("Type /help for commands, or start chatting. Ctrl+C to exit.\n", GRAY)
  );
}
...
export function startUI(
  ...
  onSlashCommand: (command: SlashCommand) => Promise<void>
): void {
  printBanner(project);
  ...
  rl.on("line", async (line: string) => {
    ...
    if (command) {
      ...
      if (command.type === "unknown") {
        console.log(
          color(`✖ Unknown command: ${command.input}. Type /help for available commands.`, RED)
        );
        rl.prompt();
        return;
      }
